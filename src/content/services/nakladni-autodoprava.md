---
title: "Nákladní autodoprava po celé ČR — do 12 tun"
description: "Spolehlivá nákladní autodoprava po celé ČR. Vozidla od 3,5 do 12 tun s hydraulickými plošinami a GPS sledováním. Hanuš Transport ze Slaného."
headline: "Nákladní autodoprava po celé České republice"
slug: "nakladni-autodoprava"
icon: "truck"
heroImage: "/images/fleet/flotila.webp"
keywords: ["autodoprava", "nákladní doprava", "přeprava zboží"]
order: 1
---

## Spolehlivá přeprava zboží po celé ČR

Hanuš Transport zajišťuje nákladní autodopravu po celé České republice. Disponujeme [6 vozidly](/vozovy-park/) od dodávek po nákladní automobily s celkovou hmotností do 12 tun. Všechna vozidla jsou vybavena hydraulickými plošinami s nosností 750 až 1 500 kg.

## Co přepravujeme

- Zboží na europaletách (kapacita až 18 palet)
- [Stavební materiály a stavebniny](/sluzby/preprava-stavebnin/)
- Průmyslové díly a komponenty
- Spotřební zboží pro maloobchodní řetězce
- [Elektrospotřebiče](/sluzby/svoz-elektrospotrebicu/) a elektroniku

## Proč Hanuš Transport

- **GPS sledování** — přesně víte, kde se vaše zásilka nachází
- **Pojištění zásilek** — každá zásilka pojištěna od nakládky po doručení
- **Hydraulické plošiny** — na všech vozidlech, snadná manipulace
- **Zkušenosti od roku 2011** — přes 12 let v oboru autodopravy
